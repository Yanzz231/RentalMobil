...
namespace TrRentals.Controllers
{
    [Route("api/trrental")]
    [ApiController]
    public class TrRentals : ControllerBase
    {
        private readonly AppDbContext _context;

        public TrRentals(AppDbContext context)
        {
            _context = context;
        }

        [HttpPost("create")]
        public async Task<IActionResult> Post([FromBody] Database.Models.TrRental request)
        {
            try
            {
        ...
        public async Task<IActionResult> GetAll([FromQuery] int? id)
        {
            try
            {
                ...
                {
                    var viewall = await _context.TrRental.ToListAsync();
        ...
        public async Task<IActionResult> Delete([FromQuery] int? id)
        {
            try
            {
                if (id.HasValue)
                {
                    var delete = await _context.TrRental.FirstOrDefaultAsync(p =>
                        p.Rental_id == id
                    );
                    if (delete == null)
                    {
                    ...
                    _context.TrRental.Remove(delete);
                    await _context.SaveChangesAsync();
